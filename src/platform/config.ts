import type { ConfigStore, SchematicFieldsConfig } from '../domain/types';
/**
 * AI原理图审查 - 配置管理
 *
 ...
 */
import { AIProvider, DEFAULT_SCHEMATIC_FIELDS } from '../domain/types';
import { isHttpUrl } from './url-validation';
...
declare global {
	// EDA may evaluate the extension bundle more than once in the same host context.
	// Keep the latest confirmed configuration shared by all of those instances.
	// eslint-disable-next-line vars-on-top
	var __aiSchReview_configState: ConfigRuntimeState | undefined;
}

function getConfigRuntimeState(): ConfigRuntimeState {
	if (!globalThis.__aiSchReview_configState) {
		globalThis.__aiSchReview_configState = {};
	}
	return globalThis.__aiSchReview_configState;
}

const runtimeState = getConfigRuntimeState();
...
export function loadConfig(): ConfigStore {
	if (runtimeState.config) {
		return {
			...runtimeState.config,
			schematicFields: mergeSchematicFields(runtimeState.config.schematicFields),
		};
	}
	try {
...
async function writeConfigValue(value: string): Promise<boolean> {
	const success = await eda.sys_Storage.setExtensionUserConfig(CONFIG_KEY, value);
	if (success) {
		return true;
	}

	// Some EDA builds return false when creating/updating a single key. Preserve all
	// other extension settings and retry through the whole-config API.
	try {
		const allConfigs = eda.sys_Storage.getExtensionAllUserConfigs?.();
		if (allConfigs && typeof allConfigs === 'object' && !Array.isArray(allConfigs)) {
			return await eda.sys_Storage.setExtensionAllUserConfigs?.({
				...allConfigs,
				[CONFIG_KEY]: value,
			}) === true;
		}
		...
		console.warn('[config] 全量配置写入兜底失败:', fallbackError);
	}
...
export async function saveConfig(config: Partial<ConfigStore>): Promise<{ success: boolean; config: ConfigStore; error?: string }> {
	...
	const merged: ConfigStore = {
		...current,
		...config,
		schematicFields: mergeSchematicFields(config.schematicFields ?? current.schematicFields),
	};
	try {
		// JSON 字符串在不同 EDA 版本间的持久化行为比复杂对象稳定。
		const success = await writeConfigValue(JSON.stringify(merged));
		if (!success) {
			return { success: false, config: current, error: '存储写入失败' };
		}
		// 不立即回读：部分 EDA 版本会短暂返回写入前的缓存值。
		runtimeState.config = merged;
		...
		if (isStorageCorruptionError(e) && await repairStorageIfCorrupted()) {
			...
			try {
				const retrySuccess = await writeConfigValue(JSON.stringify(merged));
				if (retrySuccess) {
					runtimeState.config = merged;
					return { success: true, config: merged };
				}
...
export function validateConfig(config: ConfigStore): string | null {
	...
	if (!isHttpUrl(config.apiUrl)) {
		return 'API URL必须是有效的http或https地址';
	}
	...
	if (config.mcpEnabled) {
		...
		if (!isHttpUrl(config.mcpGatewayUrl)) {
