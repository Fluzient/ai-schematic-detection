import type { SendMessageOptions } from '../ai/chat-adapter';
/**
 * AI原理图审查 - 对话模式编排器
 *
 ...
 */
import type { AbortRequest, AIBlockResponse, ChatToolCall, CollectedData, MessageBlock, RegenerateRequest, ToolEventMessage, UserMessage } from '../domain/types';
import type { CancellationController } from '../platform/cancellation';
import { ChatSession, setDebugLog } from '../ai/chat-adapter';
import { clearSessionCache, ToolOrchestrator } from '../ai/tool-orchestrator';
import { clearBackgroundNetlistState, collectSchematicData, getBackgroundNetlistState, parseNetlist, setLogToIFrame } from '../data/collector';
import { CHAT_TOPICS, ChunkType, ErrorCode, ReviewError } from '../domain/types';
import { initMcpBridge, pushSnapshot } from '../integrations/mcp-bridge';
import { createCancellationController } from '../platform/cancellation';
import { loadChatHistory, loadConfig, saveChatHistory, saveConfig, validateConfig } from '../platform/config';
import { isHttpUrl, isWebSocketUrl } from '../platform/url-validation';
...
export async function startAIChat(): Promise<void> {
	...
	try {
		...
		try {
			await eda.sys_IFrame.openIFrame('/ui/chat/index.html', CHAT_WINDOW_WIDTH, CHAT_WINDOW_HEIGHT, 'ai-sch-chat', {
				minimizeButton: true,
			});
		}
		catch {
...
function setupChatListeners(): void {
	...
	subscribe(CHAT_TOPICS.CONFIG_UPDATE, async (data: any) => {
		if (!data || typeof data !== 'object')
			return;
		const configRequestId = typeof data.requestId === 'string' ? data.requestId : undefined;
		const rejectConfigUpdate = (debugMessage: string, message: string): void => {
			publishDebugLog('warn', debugMessage);
		...
		// EDA 主上下文可能没有可用的 URL 构造器，使用协议和绝对地址格式校验。
		if (typeof data.apiUrl === 'string' && data.apiUrl.trim().length > 0 && !isHttpUrl(data.apiUrl)) {
			publishDebugLog('warn', '配置验证失败: apiUrl 格式无效');
			publishToIFrame(CHAT_TOPICS.ERROR, {
				message: '配置保存失败: API 地址必须是有效的 http 或 https 地址',
				code: 'CONFIG_VALIDATION_FAILED',
				requestId: configRequestId,
			});
			return;
		}
		if (typeof data.mcpGatewayUrl === 'string' && data.mcpGatewayUrl.trim().length > 0 && !isHttpUrl(data.mcpGatewayUrl)) {
			publishDebugLog('warn', '配置验证失败: mcpGatewayUrl 格式无效');
			publishToIFrame(CHAT_TOPICS.ERROR, {
				message: '配置保存失败: MCP Gateway 地址必须是有效的 http 或 https 地址',
				code: 'CONFIG_VALIDATION_FAILED',
				requestId: configRequestId,
			});
			return;
		}
		if (typeof data.mcpBridgeUrl === 'string' && data.mcpBridgeUrl.trim().length > 0 && !isWebSocketUrl(data.mcpBridgeUrl)) {
			publishDebugLog('warn', '配置验证失败: mcpBridgeUrl 格式无效');
			publishToIFrame(CHAT_TOPICS.ERROR, {
				message: '配置保存失败: MCP Bridge 地址必须是有效的 ws 或 wss 地址',
				code: 'CONFIG_VALIDATION_FAILED',
				requestId: configRequestId,
			});
			return;
		...
		const { requestId: _requestId, ...configUpdate } = data;
		const result = await saveConfig(configUpdate);
...
async function handleUserMessage(msg: UserMessage): Promise<void> {
	...
	try {
		...
		// 创建兼容 EDA 扩展宿主的取消控制器
		const abortController = createCancellationController();
		...
		// 如果启用 MCP，拉取工具列表
		let availableTools: import('../domain/types').ChatToolDefinition[] = [];
