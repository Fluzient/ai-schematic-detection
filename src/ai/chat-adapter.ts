...
import type { CollectedData, ConfigStore, SchematicFieldsConfig, UserMessage } from '../domain/types';
import type { CancellationSignal } from '../platform/cancellation';
import { chunkData } from '../data/chunker';
import { ChunkType, ErrorCode, ReviewError } from '../domain/types';
...
export interface SendMessageOptions {
	tools?: import('../domain/types').ChatToolDefinition[];
	onToolCalls?: (toolCalls: import('../domain/types').ChatToolCall[]) => Promise<import('../domain/types').ToolExecutionResultMessage[]>;
...
export interface ChatCompletionResult {
	...
	toolCalls: import('../domain/types').ChatToolCall[];
}
...
export interface ChatMessage {
	...
	tool_calls?: import('../domain/types').ChatToolCall[];
	tool_call_id?: string;
	name?: string;
}
...
function extractToolCalls(data: any): import('../domain/types').ChatToolCall[] {
...
function normalizeToolCalls(rawToolCalls: unknown): import('../domain/types').ChatToolCall[] {
	if (!Array.isArray(rawToolCalls)) {
		return [];
	}

	const calls: import('../domain/types').ChatToolCall[] = [];
...
function buildToolCallsFromBuffer(buffer: Map<number, { id: string; name: string; argumentsText: string }>): import('../domain/types').ChatToolCall[] {
	const calls: import('../domain/types').ChatToolCall[] = [];
