 ...
 * 原理图AI检测 - 扩展入口
 */
import { isCollectionInProgress, startAIChat, triggerBackgroundCollection } from './application/orchestrator';
import { showConfigDialog } from './platform/config';
 ...
 * 原理图AI检测入口
 */
export function schematicAiInspection(): void {
	startAIChat().catch((error) => {
		const message = error instanceof Error ? error.message : String(error);
		console.error('[schematicAiInspection] AI Chat 启动失败:', message);
		try {
			...
			console.warn('[schematicAiInspection] 启动失败对话框显示失败:', dialogError);
		}
	});
}
 ...
 * AI配置入口
 */
export function schematicAiInspectionConfig(): void {
	showConfigDialog();
}
